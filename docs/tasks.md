...
**Update 2026-08-15 (root-caused a ~24-hour notification outage: SQLite "database is locked" crashing every main-cycle run)**: User reported no Telegram messages since the previous day. `railway logs <deployment-id> --lines 4999 --filter="-getUpdates -telegram_poll -httpx"` (the 5s Telegram-poll job floods the log stream badly enough that the default 500-line fetch only covers ~2 minutes of wall-clock time — filtering it out was necessary to see anything useful; `railway logs` also caps `--lines` at ~5000 and silently returns only the *current* deployment's stream unless a specific deployment ID is passed) showed `_scheduled_cycle` raising `sqlalchemy.exc.OperationalError: (sqlite3.OperationalError) database is locked` on every single run since 2026-08-14 23:02:50 UTC — timing that lines up almost exactly with the user's last-received message, and with several redeploys made earlier that day (digest removal, `RUN_INTERVAL_MINUTES` fix, `COMPANIES_CONFIG` sync) each resetting APScheduler's next-run clock from the same restart epoch.

Root cause: `_scheduled_cycle` (2h), `_scheduled_company_retry` (1h), and `_scheduled_jsearch` (4h) are independent APScheduler jobs with `max_instances=1` *each*, but nothing prevents different jobs from running concurrently against each other (documented as intentional in `design.md` §10.1) — and since all three intervals are multiples of 1h from the same restart time, they're guaranteed to land on the exact same tick periodically. `session_scope()` (`db.py`) only commits once at the end of an entire job run, so a long job (e.g. company discovery working through 160+ companies) holds a SQLite write lock for its whole duration; `create_engine()` had no `connect_args={"timeout": ...}`, so any other job's write during that window failed immediately instead of waiting. This is not a regression from anything specific done this session — it became load-bearing because the day's other changes (companies.yaml growing to 349 entries, the 1h/2h/4h interval combination) made both the write-hold duration and the collision frequency much worse than before.

Fixed in `init_db()` (`db.py`): SQLite connections now get `connect_args={"timeout": 30}` (a generous busy-timeout so a colliding writer blocks-and-retries instead of erroring immediately) and `PRAGMA journal_mode=WAL` (readers no longer block on an in-progress writer). Guarded behind `database_url.startswith("sqlite")` so a future Postgres `DATABASE_URL` isn't affected. New `test_init_db_enables_wal_and_busy_timeout_for_a_real_sqlite_file` verifies both PRAGMAs against a real file-backed DB (`:memory:`, used by the rest of the test suite, silently ignores WAL — confirmed this doesn't error, just doesn't need separate handling). 372 tests passing, mypy/ruff clean. Deployed; confirmed no more "database is locked" in the post-deploy logs, including a case where `_scheduled_company_retry` and `_scheduled_jsearch` landed 5 seconds apart and both completed successfully.

**Update 2026-08-15 (same investigation, a second and more fundamental bug: `_is_fresh()` crashed on timezone-aware `posted_at`)**: Watching the redeployed container's logs to confirm the SQLite fix surfaced a *different*, unrelated `TypeError: can't compare offset-naive and offset-aware datetimes` in `_is_fresh()` (`main.py`), raised from `_dedupe_and_store()` on a RemoteOK posting — meaning the main cycle was still failing end-to-end (fetch/dedupe/score/notify, all of it, since `_dedupe_and_store` runs before scoring/notifying) even with the lock fix in place. Audited every scraper's `posted_at` construction: 7 of 9 (`greenhouse`, `remoteok`, `hn`, `adzuna`, `jsearch`, `yc`, `dice`) build it via `datetime.fromisoformat(...+00:00)`, which is timezone-*aware*; only `lever.py` and `indeed_rss.py` normalize it to naive UTC themselves. `_is_fresh()` compares directly against `_now()`, which is always naive — so the comparison raised `TypeError` (not returned a wrong answer) whenever *any* posting in a fetch batch came from one of those 7 sources with a real `posted_at`, aborting the whole cycle before a single alert could be sent.

This is a materially bigger finding than the SQLite lock issue: `max_posting_age_days` (the freshness filter this bug lives in) shipped 2026-08-11, so this has likely been intermittently killing cycles — silently, since a crashed `_scheduled_cycle` just logs an APScheduler error and moves on — for the better part of a week, not just the ~24h the SQLite lock issue covered. It went unnoticed because no existing test exercised a timezone-aware `posted_at` (`test_main.py`'s `_dedupe_and_store` tests all built naive datetimes via the shared `_now()` test helper) — a real test-coverage gap, not just a code bug.

Fixed centrally in `_is_fresh()` rather than touching all 7 scrapers: normalizes any tz-aware `posted_at` to naive UTC (`.astimezone(UTC).replace(tzinfo=None)`) before comparing, so it's correct regardless of which convention a given scraper (including future ones) happens to use. Added `test_dedupe_and_store_handles_timezone_aware_posted_at` reproducing the exact crash. 373 tests passing, mypy/ruff clean. Deployed.

**Combined picture**: two independent, unrelated bugs were each *fully* preventing notifications on any cycle they hit (not degrading them) — the SQLite lock issue (new, introduced by today's interval/redeploy changes) and the timezone bug (pre-existing since 2026-08-11, just now surfaced). Both are now fixed and deployed; the next few live cycles need to be watched to confirm alerts actually resume, since neither bug was directly reproducible outside the real container until this session's log archaeology surfaced them.

**Confirmed fixed, same day**: watched the deployed container through a full live main-cycle run post-fix. `_scheduled_company_retry` and `_scheduled_jsearch` landed 5 seconds apart with no "database is locked"; the main cycle completed end-to-end (10 Claude scoring calls, then exactly one real `sendMessage` → `200 OK`) with no errors of either kind. Both bugs are resolved in production, not just in tests.

**Update 2026-08-16 (SerpAPI key — and, worse, the Telegram bot token — were being logged in cleartext)**: Followed up on the `SEARCH_API_KEY` cleartext-logging note flagged two entries up. Root cause: httpx logs every request at INFO level with the full URL verbatim (`HTTP Request: %s %s "..."`, args include the `httpx.URL`), which is a separate mechanism from the app's own `_redact_chat_id()` in `notifier.py` (T-908) — that only covers messages the app logs itself, not this. Checking the actual pattern turned up a worse instance of the same problem: Telegram's bot token is embedded directly in the request path (`https://api.telegram.org/bot<TOKEN>/sendMessage`), so it's been in cleartext in *every single Telegram API log line* the whole time — a live credential that could let someone message through the bot, not just a scoped discovery-API key.

Fixed generally rather than patching just the one call site: added `_SecretRedactingFilter` (`main.py`), a `logging.Filter` attached to the `httpx` logger that fully renders each record's message (`record.getMessage()`, since httpx puts the URL in `%`-args, not the format string) and regex-redacts `api_key=...` and `/bot<id>:<token>` before the record is emitted — clears `record.args` after so the redacted text isn't re-interpolated. Covers both secrets (and any future one matching the same shape) in one place rather than requiring every call site to redact itself. Chose filtering over silencing httpx's INFO logging entirely (e.g. raising it to WARNING) specifically because the per-request status codes in those logs were exactly what made today's earlier bugs (Greenhouse/Lever fetch-abort, the SQLite lock, the timezone crash) diagnosable from `railway logs` — losing that visibility to fix a different problem would've been a bad trade. Three new tests in `test_main.py` cover both redaction patterns and confirm non-secret URLs pass through unchanged. 376 tests passing, mypy/ruff clean.

**Update 2026-08-16 (BeBee and JobLeads apply links excluded, even when JSearch marks them "direct")**: Per explicit user request, after alerts resumed following the notification-outage fix — BeBee and JobLeads links were showing up as the "Apply" button despite `_direct_apply_url()` (T-13 §4.1a) supposedly already preferring the employer's own page. Root cause: JSearch's own `is_direct`/`job_apply_is_direct` flags have been observed marking BeBee and JobLeads as direct even though they're just another job-board re-poster, not the employer — the existing preference logic trusted that flag completely, so a bad flag from the data provider let them through anyway.

Added `_BLOCKED_APPLY_DOMAINS = {"bebee.com", "jobleads.com"}` and rewrote `_direct_apply_url()` to build the full priority-ordered candidate chain first (direct `job_apply_link` → direct `apply_options` entries → fallback `job_apply_link` → any remaining `apply_options` entry, same order as before) and only then filter out blocked domains — so a blocked link never displaces a genuinely better candidate further down the chain, but if literally nothing else is available it's still returned rather than leaving the posting with no apply link at all. 4 new tests in `test_scrapers_jsearch.py`, including one confirming the "marked direct anyway" scenario and one confirming the last-resort fallback still works. 379 tests passing, mypy/ruff clean.
